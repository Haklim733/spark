...
uv run -m src.legal.generate_legal_docs --bucket=raw --key=/docs/legal --num-docs=100
TODAY=$(date -u +%Y%m%d) 
# uv run -m src.process_legal --file-dir="s3a://raw/docs/legal/*/$TODAY/**/*" --table-name=legal.documents_snapshot --mode=batch --branch=staging
